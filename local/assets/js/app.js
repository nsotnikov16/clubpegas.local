...
function setLeftPromotion(scroll) {
    if (firstStock) {
        ...
        if (scroll) {
            ...
            firstStockCopy.classList.add('display')
        }
...
if (pageSubtitle && pageSubtitle.innerHTML != '') {
    document.querySelector('.page-subtitle').innerHTML = pageSubtitle.innerHTML
}

const titles = document.querySelectorAll('.txt')

function replaceColorText(array) {
    array.forEach(item => {
        item.innerHTML = item.innerHTML.replace(/=#/gi, '<span class="color">')
        item.innerHTML = item.innerHTML.replace(/#=/gi, '</span>')
    })
}
if (titles.length > 0) {
    replaceColorText(titles);
}

const txtTitleColor = document.querySelector('#txtTitleColor')
if (txtTitleColor && txtTitleColor.textContent !== '') {
    const words = txtTitleColor.textContent.split(',')
    const title = document.querySelector('.titleTxtColor')
    if (title) {
        title.innerHTML = title.innerHTML.toLowerCase()
        words.forEach(w => {
            w = w.trim().toLowerCase()
            if (title.innerHTML.includes(w)) {
                title.innerHTML = title.innerHTML.replace(`${w}`, `<span class="color">${w}</span>`)
            }
        })
    }
}
