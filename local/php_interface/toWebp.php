...
class Pict
{
	...
	public static function resizePict($file, $width, $height, $isProportional = true,  $intQuality = 70, $watermark = false)
	{

		$file = \CFile::ResizeImageGet($file, array('width' => $width, 'height' => $height), ($isProportional ? BX_RESIZE_IMAGE_PROPORTIONAL : BX_RESIZE_IMAGE_EXACT), false, $watermark, false, $intQuality);

		return $file['src'];
	}

	public static function getResizeWebp($file, $width, $height, $isProportional = true, $intQuality = 70, $watermark = false)
	{
		$file['SRC'] = self::resizePict($file, $width, $height, $isProportional, $intQuality, $watermark);
